...
private:
    ...
    void handle_bestmove(int client_fd, const std::string& body) {
        try {
            ...
            std::string fen = body_json["fen"];
            int depth = body_json.value("depth", 4);
            ...
            std::cout << "Debug: Best move from engine: start=" << (int)best_move.start() << " end=" << (int)best_move.end() << " flag=" << (int)best_move.flag() << std::endl;
            
            // Convert move to response format
            auto response_json = ChessAdapter::move_to_json(best_move, board);
            std::cout << "Debug: Response JSON created successfully" << std::endl;
            
            auto score_json = ChessAdapter::score_to_json(0); // TODO: Get actual score from engine
            std::cout << "Debug: Score JSON created successfully" << std::endl;
            
            json final_response;
            final_response["bestMove"] = response_json;
            final_response["score"] = score_json;
            
            std::cout << "Debug: Final response created, sending..." << std::endl;
            send_response(client_fd, final_response.dump());
    ...
    void handle_move(int client_fd, const std::string& body) {
        try {
            ...
            if (!body_json.contains("move") || !body_json["move"].is_string()) {
                send_error(client_fd, 400, "Missing or invalid 'move' field");
                return;
            }
            
            std::string fen = body_json["fen"];
            std::string move_str = body_json["move"];
            ...
            std::cout << "Debug: Parsed move: start=" << (int)move->start() << " end=" << (int)move->end() << " flag=" << (int)move->flag() << std::endl;
            std::cout << "Debug: Move raw value: " << move->move << std::endl;
            std::cout << "Debug: Number of legal moves: " << legal_moves.size() << std::endl;
            
            bool is_legal = false;
            for (const auto& legal_move : legal_moves) {
                std::cout << "Debug: Legal move: start=" << legal_move.start() << " end=" << legal_move.end() << " flag=" << (int)legal_move.flag() << std::endl;
                if (legal_move.start() == move->start() && 
                    legal_move.end() == move->end() && 
                    legal_move.flag() == move->flag()) {
                    is_legal = true;
                    std::cout << "Debug: Move matched!" << std::endl;
                    break;
                }
            }
